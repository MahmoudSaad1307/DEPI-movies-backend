...
router.patch("/:id/watched", async (req, res) => {
  try {
    ...
    const userId = req.params.id;
    const { movieId ,rating,ratingProvided=true} = req.body;
    ...
    if (user.movies.watched.some(movie => movie.movieId === movieId)&&noRating) {
    ...
    } else if(user.movies.watched.some(movie => movie.movieId === movieId)&&!no){
      // user.movies.watchlist=[{}]
